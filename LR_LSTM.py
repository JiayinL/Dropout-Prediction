...
class LR_LSTM(nn.Module):
    
    def __init__(self):
        super(LR_LSTM, self).__init__()  
        ...
        self.bilstm = nn.LSTM(input_size = feature_size2, hidden_size = hidden_dim // 2, num_layers=num_of_lstm_layer,  bidirectional=True)
...
#     loss_value = []
    for batchID in range(numOfMinibatches):
        ...
        predictions,LR_result,LSTM_result = model(courseid_LR,courseid_CNN,continuesfeature1,continuesfeature2,videoid)
        

        predictions = torch.flatten(predictions)
        LR_result = torch.flatten(LR_result)
        LSTM_result = torch.flatten(LSTM_result)

        loss_final = MSELoss(predictions,y[leftIndex: rightIndex].float())
        loss_lr = MSELoss(LR_result,y[leftIndex: rightIndex].float())
        loss_lstm = MSELoss(LSTM_result,y[leftIndex: rightIndex].float())
#         print('loss: ',loss)
        loss = loss_final + loss_lr +loss_lstm
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
#             ground_truth = ground_truth.detach().numpy().tolist()
            ...
            print('Epoch[{}/{}],loss:{:.4f},loss_final:{:.4f},loss_LR:{:.4f},loss_GRU:{:.4f},acc:{:.4f}'.format(epoach, epoach_count,loss.item(),loss_final.item(),loss_lr.item(),loss_lstm.item(),acc))
...
#     for j in range(18720):
#         prediction= model(course_id[j:j+10],video_id[j:j+10],continues_feature[j:j+10])


torch.save(model.state_dict(), 'lr_lstm.model')
