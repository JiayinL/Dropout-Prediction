...
sequence_len = 70
# in_channel = 
feature_size = course_emb_size + video_emb_size + 13
hidden_dim = 64
num_of_lstm_layer = 1
...
#         self.bi_gru = nn.GRU(input_size = feature_size, hidden_size = hidden_dim // 2, num_layers=num_of_lstm_layer,  bidirectional=True)
...
# #         print('info_fusion: ',output.shape)
        
#         info_fusion = self.tanh_activation(self.fc1(output))
        
# #         print('info_fusion: ',info_fusion.shape)
...
#     loss_value = []
    for batchID in range(numOfMinibatches):
        ...
        predictions = model(courseid,videoid,continuesfeature)
...
#                 print('test_leftIndex: ',test_leftIndex)
                test_predictions = model(test_courseid,test_videoid,test_continuesfeature)
